...
void PairAEAM::compute(int eflag, int vflag)
{
	...
	for (ii = 0; ii < inum; ii++) {
		...
		ztmp = x[i][2];
			...
			for (jj = 0; jj < jnum; jj++) {
				...
				if (itype>setfl->nnonangular && jtype>setfl->nnonangular) CutDec=1.5;
				else CutDec=0;
				...
				if (itype<=setfl->nnonangular){							//atom I is noangular
					...
					for (kk = jj+1; kk < jnum; kk++) {
						...
						if (ktype>setfl->nnonangular) CutDec=1.5;						//atom I and K are angular
						else CutDec=0;
	...
	for (ii = 0; ii < inum; ii++) {
		...
		if (eflag) {
			poteam = ((coeff[3]*p + coeff[4])*p + coeff[5])*p + coeff[6];		//F(rhoi) for all atoms F(sum(fij)) for Me F(2sum(fij.fik.ftet)^0.5 for Si
			if (eflag_global) eng_vdwl += poteam;
			if (eflag_atom) 
				if (itype<=nnonangular) eatom[i] += poteam;
				 else eatom[i] +=THIRD*poteam;
		}
	...
	for (ii = 0; ii < inum; ii++) {
		...
		if ( rho[i]>minrho)
			Fptmp = ni*pow(rho[i],(ni-1));										//for Me Fptmp=1  for Si Fptmp=0.5(2sum(fij.fik.ftet)))^(-0.5)=0.5/rho(i)
		else
			Fptmp=0;
		...
		for (jj = 0; jj < jnum; jj++) {
			...
			if (itype<=setfl->nnonangular) continue;							//atom I is Au
				...
				for (kk = jj+1; kk < jnum; kk++) {
					...
					if (ktype>setfl->nnonangular) CutDec=1.5;
					else CutDec=0;
...
void PairAEAM::coeff(int narg, char **arg)
{
	...
	for (i = 3; i < narg; i++) {
		...
		else error->all(FLERR,"No matching element in AEAM potential file");
	}
...
void PairAEAM::read_file(char *filename)
{
   ...
   for (i = 2; i < file->nelements; i++) {
	...
	if (me == 0) {
		...
		sscanf(line,"%d %lg %lg",
			   &file->nrho[i],&file->drho[i],&file->mass[i]);
	}
	...
	for (j = 0; j < file->nelements; j++) {
		if (me == 0) {
			fgets(line,MAXLINE,fptr);
			sscanf(line,"%d %lg %lg",
				   &file->nr[i][j],&file->dr[i][j],&file->cut[i][j]);
		}
